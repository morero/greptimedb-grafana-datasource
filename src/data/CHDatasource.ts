...
function createLogLevelAggregate(llfColumn: string, searchTermsString: string, alias: string): { aggregateType: AggregateType; column: string; alias: string } {
  ...
  return {
    ...
    // LOWER() enables case-insensitive matching with only lowercase IN values,
    // cutting the clause size by ~3x vs the old triple-case approach.
    column: `CASE WHEN LOWER(${llfColumn}) IN (${inClause}) THEN 1 ELSE 0 END`,
    alias: alias,
  };
}
